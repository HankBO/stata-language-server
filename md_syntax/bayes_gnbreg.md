...
| For a detailed description of `options`, see [<var class="command">Options for gnbreg</var><strong></strong>](gnbreg##options_gnbreg) in [<strong>[R]</strong> nbreg](http://www.stata.com/help.cgi?nbreg). |                       |                                                                    |
...
| Model parameters are regression coefficients `{c -(}depvar:indepvars{c )-}` for the main regression and `{c -(}lnalpha:varlist{c )-}` for the log-dispersion equation. Use the `dryrun` option to see the definitions of model parameters prior to estimation.                                                           |     |     |
