...
| For a detailed description of `options`, see [<var class="command">Options</var><strong></strong>](oprobit##options) in [<strong>[R]</strong> oprobit](http://www.stata.com/help.cgi?oprobit). |                   |                                                              |
...
| Model parameters are regression coefficients `{c -(}depvar:indepvars{c )-}` and cutpoints `{c -(}cut1{c )-}`, `{c -(}cut2{c )-}`, and so on. Use the `dryrun` option to see the definitions of model parameters prior to estimation.                                                                                         |     |     |
