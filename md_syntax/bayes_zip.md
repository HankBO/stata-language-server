...
| For a detailed description of `options`, see [<var class="command">Options</var><strong></strong>](zip##options) in [<strong>[R]</strong> zip](http://www.stata.com/help.cgi?zip). |                       |                                                                    |
...
| Model parameters are regression coefficients `{c -(}depvar:indepvars{c )-}` for the main regression and `{c -(}inflate:varlist{c )-}` for the inflation equation. Use the `dryrun` option to see the definitions of model parameters prior to estimation.                                                                |     |     |
